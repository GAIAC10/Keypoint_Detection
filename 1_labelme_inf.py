"""
标注前，需开启Save Automatically自动保存
标注前，需关闭Save With Image Data将图像本身保存至标签文件中
...
for each_ann in labelme['shapes']:  # 遍历每一个标注
    if each_ann['shape_type'] == 'point':  # 筛选出关键点标注
        ...
        img_bgr = cv2.putText(
            ...
            cv2.FONT_HERSHEY_SIMPLEX, kpt_labelstr['font_size'],
            kpt_color,
...
for each_ann in labelme['shapes']:  # 遍历每一个标注
    if each_ann['shape_type'] == 'polygon':  # 筛选出多段线（polygon）标注
        ...
        img_bgr = cv2.putText(img_bgr,
                              poly_label,
                              (
                                  x_mean + poly_labelstr['offset_x'],
                                  y_mean + poly_labelstr['offset_y']
                              ),
                              cv2.FONT_HERSHEY_SIMPLEX,
                              poly_labelstr['font_size'],
                              poly_color,
...
plt.imshow(img_bgr[:,:,::-1])
plt.show()
cv2.imwrite('./dataset/visualize.jpg', img_bgr)

"""
labelme格式数据集下载
https://zihao-download.obs.cn-east-3.myhuaweicloud.com/yolov8/datasets/Triangle_215_Dataset/Triangle_215_Keypoint_Labelme.zip
"""